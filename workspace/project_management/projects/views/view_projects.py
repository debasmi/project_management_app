...
class ProjectUpdateView(LoginRequiredMixin,View):
    ...
    def get(self,request,pk):
        ...
        if admin_or_manager:
            ...
            messages.warning(request,"you do not have permission to access the page")
            return redirect('project_list')
        return render(request,self.template_name,{'project':project})
    
    def post(self,reqeust,pk):
        project=get_object_or_404(Project,pk=pk)
        project.name = reqeust.POST.get('name')
        project.description = reqeust.POST.get('description')
        project.start_date = reqeust.POST.get('start_date')
        project.end_date = reqeust.POST.get('end_date')
        project.save()
        return redirect('project_list')
